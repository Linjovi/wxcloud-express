      ...
      <div className="flex flex-col items-center mt-6">
        ...
        <div className="text-center space-y-2 mt-6">
            ...
            <div className="bg-orange-50 text-orange-800 text-sm px-4 py-3 rounded-xl inline-block font-medium mx-4 italic relative border border-orange-100 shadow-sm mt-2">
                <span className="absolute -top-3 -left-2 text-3xl text-orange-200">❝</span>
                 {result.funnyComment}
                <span className="absolute -bottom-4 -right-1 text-3xl text-orange-200 leading-none">❞</span>
            </div>
        </div>
      </div>
        ...
        <div className="space-y-5">
            ...
            <div>
                <div className="flex justify-between text-sm mb-1.5">
                    <span className={`font-bold ${isWinnerA ? 'text-orange-600' : 'text-gray-600'}`}>
                        {inputData.nameA}
                    </span>
                    <span className="font-mono font-bold text-gray-700">{result.scoreA}</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                    <div 
                        className={`h-full rounded-full transition-all duration-1000 ease-out ${isWinnerA ? 'bg-gradient-to-r from-orange-400 to-red-500' : 'bg-gray-300'}`} 
            ...
            <div>
                <div className="flex justify-between text-sm mb-1.5">
                    <span className={`font-bold ${isWinnerB ? 'text-orange-600' : 'text-gray-600'}`}>
                        {inputData.nameB}
                    </span>
                    <span className="font-mono font-bold text-gray-700">{result.scoreB}</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                    <div 
                        className={`h-full rounded-full transition-all duration-1000 ease-out ${isWinnerB ? 'bg-gradient-to-r from-orange-400 to-red-500' : 'bg-gray-300'}`} 
      ...
      <div className="bg-gradient-to-br from-orange-50 to-white rounded-2xl shadow-sm p-5 border border-orange-100">
        ...
        <ul className="space-y-4">
          {result.actionItems.map((item, idx) => (
            <li key={idx} className="flex items-start text-sm text-gray-700 group">
        ...
        >
            {copied ? (
                ...
                <>
                    ...
                    <span>复制给 {isWinnerA ? inputData.nameB : inputData.nameA} 看</span>
                </>
            )}
        </button>
